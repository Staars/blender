...
const string MetalDevice::get_metal_program_name(const string &kernel_name)
{
...
const string MetalDevice::get_metal_program_filename(const string &kernel_name)
{
...
void MetalDevice::enable_default_features(DeviceRequestedFeatures &features)
{
...
string MetalDevice::get_build_options(const DeviceRequestedFeatures &requested_features,
                                       const string &metal_program_name,
                                       bool preview_kernel)
{
  /* first check for non-split kernel programs */
  if (metal_program_name == "base" || metal_program_name == "denoising") {
    return "";
  }
  else if (metal_program_name == "bake") {
  ...
  else if (metal_program_name == "displace") {
  ...
  else if (metal_program_name == "background") {
  ...
  if (preview_kernel) {
  ...
  else if (metal_program_name == "split_do_volume" && !requested_features.use_volume) {
    ...
    if (metal_program_name == "split_bundle") {
...
MetalDevice::MetalSplitPrograms::MetalSplitPrograms(MetalDevice *device_)
{
  device = device_;
}

MetalDevice::MetalSplitPrograms::~MetalSplitPrograms()
{
...
void MetalDevice::MetalSplitPrograms::load_kernels(
    vector<MetalProgram *> &programs,
...
#  define ADD_SPLIT_KERNEL_PROGRAM(kernel_name) \
    ...
    program_##kernel_name = MetalDevice::MetalProgram( \
        device, \
     ...
     * Blender processes. */
    program_split = MetalDevice::MetalProgram(
        device,
        "split_bundle",
...
/* Copy dummy KernelGlobals related to Metal from kernel_globals.h to
 * fetch its size.
 */
...
class MetalSplitKernelFunction : public SplitKernelFunction {
 public:
  MetalDevice *device;
  MetalDevice::MetalProgram program;
  CachedSplitMemory &cached_memory;
  int cached_id;

  MetalSplitKernelFunction(MetalDevice *device, CachedSplitMemory &cached_memory)
      : device(device), cached_memory(cached_memory), cached_id(cached_memory.id - 1)
  {
  }

  ~MetalSplitKernelFunction()
  {
    program.release();
  }
  ...
  virtual bool enqueue(const KernelDimensions &dim, device_memory &kg, device_memory &data)
  {
    ...
    device->metal_assert_err(device->ciErr, "clEnqueueNDRangeKernel");

    if (device->ciErr != CL_SUCCESS) {
      string message = string_printf("Metal error: %s in clEnqueueNDRangeKernel()",
                                     clewErrorString(device->ciErr));
      device->metal_error(message);
      return false;
    }
...
class MetalSplitKernel : public DeviceSplitKernel {
 ...
 public:
  explicit MetalSplitKernel(MetalDevice *device) : DeviceSplitKernel(device), device(device)
  {
  }
  ...
  {
    MetalSplitKernelFunction *kernel = new MetalSplitKernelFunction(device, cached_memory);

    const string program_name = device->get_metal_program_name(kernel_name);
    kernel->program = MetalDevice::MetalProgram(
        device,
        program_name,
        device->get_metal_program_filename(kernel_name),
  ...
  virtual uint64_t state_buffer_size(device_memory &kg, device_memory &data, size_t num_threads)
  {
    ...
    MetalDevice::MetalSplitPrograms *programs = device->get_split_programs();
...
//    if (DebugFlags().metal.mem_limit) {
//      max_buffer_size = min(max_buffer_size,
//                            cl_ulong(DebugFlags().metal.mem_limit - device->stats.mem_used));
//    }
...
bool MetalDevice::metal_error(cl_int err)
{
  if (err != CL_SUCCESS) {
    string message = string_printf("Metal error (%d): %s", err, clewErrorString(err));
    if (error_msg == "")
      error_msg = message;
    fprintf(stderr, "%s\n", message.c_str());
    return true;
...
void MetalDevice::metal_error(const string &message)
{
  if (error_msg == "")
    error_msg = message;
  fprintf(stderr, "%s\n", message.c_str());
...
void MetalDevice::metal_assert_err(cl_int err, const char *where)
{
  if (err != CL_SUCCESS) {
    string message = string_printf(
        "Metal error (%d): %s in %s", err, clewErrorString(err), where);
    if (error_msg == "")
      error_msg = message;
    fprintf(stderr, "%s\n", message.c_str());
#  ifndef NDEBUG
...
MetalDevice::MetalDevice(DeviceInfo &info, Stats &stats, Profiler &profiler, bool background)
      ...
      texture_info(this, "__texture_info", MEM_GLOBAL)
{
  ...
  if (usable_devices.size() == 0) {
    metal_error("Metal: no devices found.");
    VLOG(1) << "Metal: no devices found.";
    return;
  }
  assert(info.num < usable_devices.size());
  MetalPlatformDevice &platform_device = usable_devices[info.num];
  device_num = info.num;
  cpPlatform = NULL;
  cdDevice = platform_device.device_id;
  platform_name = platform_device.platform_name;
  device_name = platform_device.device_name;
  VLOG(2) << "Creating new Cycles device for Metal platform " << platform_name << ", device "
          << device_name << ".";

  {
    /* try to use cached context */
    thread_scoped_lock cache_locker;
//    cxContext = MetalCache::get_context(cpPlatform, cdDevice, cache_locker);
...
//          context_props, 1, &cdDevice, context_notify_callback, cdDevice, &ciErr);
      ...
      if (metal_error(ciErr)) {
        metal_error("Metal: clCreateContext failed");
        return;
      }

      /* cache it */
//      MetalCache::store_context(cpPlatform, cdDevice, cxContext, cache_locker);
    }
  }

//  cqCommandQueue = clCreateCommandQueue(cxContext, cdDevice, 0, &ciErr);
  if (metal_error(ciErr)) {
    metal_error("Metal: Error creating command queue");
    return;
  }
  ...
  split_kernel = new MetalSplitKernel(this);
...
void CL_CALLBACK MetalDevice::context_notify_callback(const char *err_info,
                                                       const void * /*private_info*/,
                                                       size_t /*cb*/,
                                                       void *user_data)
{
//  string device_name = MetalInfo::get_device_name((cl_device_id)user_data);
//  fprintf(stderr, "Metal error (%s): %s\n", device_name.c_str(), err_info);
}

bool MetalDevice::metal_version_check()
{
//  string error;
//  if (!MetalInfo::platform_version_check(cpPlatform, &error)) {
//    metal_error(error);
//    return false;
//  }
//  if (!MetalInfo::device_version_check(cdDevice, &error)) {
//    metal_error(error);
//    return false;
//  }
  return true;
...
string MetalDevice::device_md5_hash(string kernel_custom_build_options)
{
  MD5Hash md5;
...
bool MetalDevice::load_kernels(const DeviceRequestedFeatures &requested_features)
{
  ...
  if (!device_initialized) {
    fprintf(stderr, "Metal: failed to initialize device.\n");
    return false;
  }

  /* Verify we have right metal version. */
  if (!metal_version_check())
    return false;
  ...
  if (!requested_features.use_baking && requested_features.use_denoising) {
    denoising_program = MetalProgram(
   ...
   * processes to workaround Metal frameworks serializing the calls
   * internally within a single process. */
  foreach (MetalProgram *program, programs) {
    if (!program->load()) {
...
void MetalDevice::load_required_kernels(const DeviceRequestedFeatures &requested_features)
{
  vector<MetalProgram *> programs;
  base_program = MetalProgram(
  ...
  if (requested_features.use_background_light) {
    background_program = MetalProgram(this,
                                       "background",
  ...
  foreach (MetalProgram *program, programs) {
    if (!program->load()) {
      load_required_kernel_task_pool.push(function_bind(&MetalProgram::compile, program));
    }
  }
}

void MetalDevice::load_preview_kernels()
{
...
bool MetalDevice::wait_for_availability(const DeviceRequestedFeatures &requested_features)
{
  if (requested_features.use_baking) {
...
MetalDevice::MetalSplitPrograms *MetalDevice::get_split_programs()
{
  return use_preview_kernels ? &preview_programs : &kernel_programs;
}

DeviceKernelStatus MetalDevice::get_active_kernel_switch_state()
{
...
//  if (DebugFlags().metal.mem_limit) {
//    max_alloc_size = min(max_alloc_size, cl_ulong(DebugFlags().metal.mem_limit - stats.mem_used));
//  }
   ...
   * supported by Metal. Using NULL as device pointer also doesn't really
  ...
  if (size != 0) {
    ...
    metal_assert_err(ciErr, "clCreateBuffer");
  }
  else {
...
void MetalDevice::mem_copy_to(device_memory &mem)
{
  if (mem.type == MEM_GLOBAL) {
    ...
    size_t size = mem.memory_size();
    if (size != 0) {
      metal_assert(clEnqueueWriteBuffer(cqCommandQueue,
...
void MetalDevice::mem_copy_from(device_memory &mem, int y, int w, int h, int elem)
{
  ...
  assert(size != 0);
  metal_assert(clEnqueueReadBuffer(cqCommandQueue,
                                    CL_MEM_PTR(mem.device_pointer),
                                    CL_TRUE,
                                    offset,
                                    size,
...
void MetalDevice::mem_zero_kernel(device_ptr mem, size_t size)
{
  ...
  while (d_offset < size) {
    ...
    metal_assert_err(ciErr, "clEnqueueNDRangeKernel");
  ...
  if (mem.type == MEM_GLOBAL) {
    ...
    if (mem.device_pointer) {
      if (mem.device_pointer != 0) {
        metal_assert(clReleaseMemObject(CL_MEM_PTR(mem.device_pointer)));
      }
      mem.device_pointer = 0;
...
int MetalDevice::mem_sub_ptr_alignment()
{
  return 0;
//  return MetalInfo::mem_sub_ptr_alignment(cdDevice);
}
...
device_ptr MetalDevice::mem_alloc_sub_ptr(device_memory &mem, int offset, int size)
{
  ...
  metal_assert_err(ciErr, "clCreateSubBuffer");
  return sub_buf;
}

void MetalDevice::mem_free_sub_ptr(device_ptr device_pointer)
{
  if (device_pointer != 0) {
    metal_assert(clReleaseMemObject(CL_MEM_PTR(device_pointer)));
  }
}
...
size_t MetalDevice::global_size_round_up(int group_size, int global_size)
{
  int r = global_size % group_size;
  return global_size + ((r == 0) ? 0 : group_size - r);
}

void MetalDevice::enqueue_kernel(
...
//      cdDevice, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(size_t) * 3, max_work_items, NULL);
  ...
  metal_assert(clFlush(cqCommandQueue));
}

void MetalDevice::set_kernel_arg_mem(cl_kernel kernel, cl_uint *narg, const char *name)
{
  cl_mem ptr;
  ...
  metal_assert(clSetKernelArg(kernel, (*narg)++, sizeof(ptr), (void *)&ptr));
}

void MetalDevice::set_kernel_arg_buffers(cl_kernel kernel, cl_uint *narg)
{
...
void MetalDevice::flush_texture_buffers()
{
...
#  include "kernel/kernel_textures.h"
  ...
  foreach (texture_slot_t &slot, texture_slots) {
    ...
    MetalMemoryManager::BufferDescriptor desc = memory_manager.get_descriptor(slot.name);
...
void MetalDevice::film_convert(DeviceTask &task,
                                device_ptr buffer,
...
bool MetalDevice::denoising_non_local_means(device_ptr image_ptr,
...
bool MetalDevice::denoising_construct_transform(DenoisingTask *task)
{
...
bool MetalDevice::denoising_accumulate(device_ptr color_ptr,
...
bool MetalDevice::denoising_solve(device_ptr output_ptr, DenoisingTask *task)
{
...
bool MetalDevice::denoising_combine_halves(device_ptr a_ptr,
                                            device_ptr b_ptr,
                                            device_ptr mean_ptr,
                                            device_ptr variance_ptr,
                                            int r,
...
bool MetalDevice::denoising_divide_shadow(device_ptr a_ptr,
                                           device_ptr b_ptr,
...
bool MetalDevice::denoising_get_feature(int mean_offset,
                                         int variance_offset,
                                         device_ptr mean_ptr,
                                         device_ptr variance_ptr,
                                         float scale,
...
bool MetalDevice::denoising_write_feature(int out_offset,
                                           device_ptr from_ptr,
...
bool MetalDevice::denoising_detect_outliers(device_ptr image_ptr,
                                             device_ptr variance_ptr,
...
void MetalDevice::denoise(RenderTile &rtile, DenoisingTask &denoising)
{
  denoising.functions.construct_transform = function_bind(
      &MetalDevice::denoising_construct_transform, this, &denoising);
  denoising.functions.accumulate = function_bind(
      &MetalDevice::denoising_accumulate, this, _1, _2, _3, _4, &denoising);
  denoising.functions.solve = function_bind(&MetalDevice::denoising_solve, this, _1, &denoising);
  denoising.functions.divide_shadow = function_bind(
      &MetalDevice::denoising_divide_shadow, this, _1, _2, _3, _4, _5, &denoising);
  denoising.functions.non_local_means = function_bind(
      &MetalDevice::denoising_non_local_means, this, _1, _2, _3, _4, &denoising);
  denoising.functions.combine_halves = function_bind(
      &MetalDevice::denoising_combine_halves, this, _1, _2, _3, _4, _5, _6, &denoising);
  denoising.functions.get_feature = function_bind(
      &MetalDevice::denoising_get_feature, this, _1, _2, _3, _4, _5, &denoising);
  denoising.functions.write_feature = function_bind(
      &MetalDevice::denoising_write_feature, this, _1, _2, _3, &denoising);
  denoising.functions.detect_outliers = function_bind(
      &MetalDevice::denoising_detect_outliers, this, _1, _2, _3, _4, &denoising);
...
void MetalDevice::shader(DeviceTask &task)
{
  /* cast arguments to cl types */
  cl_mem d_data = CL_MEM_PTR(const_mem_map["__data"]->device_pointer);
  ...
  MetalDevice::MetalProgram *program = &background_program;
...
void MetalDevice::bake(DeviceTask &task, RenderTile &rtile)
{
...
string MetalDevice::kernel_build_options(const string *debug_src)
{
  ...
  if (platform_name == "Apple")
     ...
     */
    if (MetalInfo::use_debug() && debug_src)
...
//    build_options += "-D__KERNEL_METAL_DEBUG__ ";
//  }
 ...
 */
int MetalDevice::kernel_set_args(cl_kernel kernel,
...
void MetalDevice::release_kernel_safe(cl_kernel kernel)
{
  if (kernel) {
...
void MetalDevice::release_mem_object_safe(cl_mem mem)
{
  if (mem != NULL) {
...
void MetalDevice::release_program_safe(cl_program program)
{
  if (program) {
...
cl_program MetalDevice::load_cached_kernel(ustring key, thread_scoped_lock &cache_locker)
{
  return NULL;
//  return MetalCache::get_program(cpPlatform, cdDevice, key, cache_locker);
}

void MetalDevice::store_cached_kernel(cl_program program,
                                       ustring key,
                                       thread_scoped_lock &cache_locker)
{
//  MetalCache::store_program(cpPlatform, cdDevice, program, key, cache_locker);
}

Device *metal_create_split_device(DeviceInfo &info,
                                   Stats &stats,
                                   Profiler &profiler,
                                   bool background)
{
