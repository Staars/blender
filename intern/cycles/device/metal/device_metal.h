...
#  include "device/metal/memory_manager.h"
...
struct MetalPlatformDevice {
  MetalPlatformDevice(const string &platform_name,
                       uint64_t device_id,
...
/* Contains all static Metal helper functions. */
class MetalInfo {
 public:
...
class MetalCache {
  ...
  /* lazy instantiate */
  static MetalCache &global_instance();
...
#  define metal_device_assert(device, stmt) \
    { \
      cl_int err = stmt; \
\
      if (err != CL_SUCCESS) { \
        string message = string_printf( \
            "Metal error: %s in %s (%s:%d)", clewErrorString(err), #stmt, __FILE__, __LINE__); \
...
class MetalDevice : public Device {
 public:
  ...
  class MetalProgram {
   public:
    MetalProgram() : loaded(false), needs_compiling(true), program(NULL), device(NULL)
    {
    }
    MetalProgram(MetalDevice *device,
    ...
    void compile();
    /* Create the Metal kernels after loading or compiling */
    void create_kernels();
   ...
   private:
     ...
     * This is required for multithreaded Metal compilation, since most Frameworks serialize
    ...
    /* Build the program by calling Metal directly. */
  ...
  class MetalSplitPrograms {
   public:
    MetalDevice *device;
    MetalProgram program_split;
    MetalProgram program_lamp_emission;
    MetalProgram program_do_volume;
    MetalProgram program_indirect_background;
    MetalProgram program_shader_eval;
    MetalProgram program_holdout_emission_blurring_pathtermination_ao;
    MetalProgram program_subsurface_scatter;
    MetalProgram program_direct_lighting;
    MetalProgram program_shadow_blocked_ao;
    MetalProgram program_shadow_blocked_dl;

    MetalSplitPrograms(MetalDevice *device);
    ~MetalSplitPrograms();

    /* Load the kernels and put the created kernels in the given
     * `programs` parameter. */
    void load_kernels(vector<MetalProgram *> &programs,
  ...
  MetalProgram base_program;
  MetalProgram bake_program;
  MetalProgram displace_program;
  MetalProgram background_program;
  MetalProgram denoising_program;

  MetalSplitPrograms kernel_programs;
  MetalSplitPrograms preview_programs;
  ...
  bool metal_error(cl_int err);
  void metal_error(const string &message);
  void metal_assert_err(cl_int err, const char *where);

  MetalDevice(DeviceInfo &info, Stats &stats, Profiler &profiler, bool background);
  ~MetalDevice();
  ...
  bool metal_version_check();
  MetalSplitPrograms *get_split_programs();
  ...
  /* Get the name of the metal program for the given kernel */
  const string get_metal_program_name(const string &kernel_name);
  /* Get the program file name to compile (*.cl) for the given kernel */
  const string get_metal_program_filename(const string &kernel_name);
 ...
 private:
  MetalMemoryManager memory_manager;
