...
ccl_device_inline Transform object_fetch_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                   int object,
...
ccl_device_inline Transform lamp_fetch_transform(METAL_ASQ_DEVICE const KernelGlobals *kg, int lamp, bool inverse)
{
  if (inverse) {
...
ccl_device_inline Transform object_fetch_motion_pass_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                               int object,
...
ccl_device_inline Transform object_fetch_transform_motion(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                          int object,
                                                          float time)
{
...
ccl_device_inline Transform object_fetch_transform_motion_test(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                               int object,
                                                               float time,
                                                               METAL_ASQ_THREAD Transform *itfm)
{
...
ccl_device_inline Transform object_get_transform(METAL_ASQ_DEVICE const KernelGlobals *kg, METAL_ASQ_DEVICE const ShaderData *sd)
{
#ifdef __OBJECT_MOTION__
...
ccl_device_inline Transform object_get_inverse_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
...
ccl_device_inline void object_position_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                 METAL_ASQ_DEVICE const ShaderData *sd,
...
ccl_device_inline void object_inverse_position_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                         METAL_ASQ_DEVICE const ShaderData *sd,
                                                         METAL_ASQ_THREAD float3 *P)
{
#ifdef __OBJECT_MOTION__
...
ccl_device_inline void object_inverse_normal_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                       METAL_ASQ_DEVICE const ShaderData *sd,
...
ccl_device_inline void object_normal_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                               METAL_ASQ_DEVICE const ShaderData *sd,
                                               METAL_ASQ_THREAD float3 *N)
{
#ifdef __OBJECT_MOTION__
...
ccl_device_inline void object_dir_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                            METAL_ASQ_DEVICE const ShaderData *sd,
                                            METAL_ASQ_THREAD float3 *D)
{
#ifdef __OBJECT_MOTION__
  if (sd->object_flag & SD_OBJECT_MOTION) {
...
ccl_device_inline void object_inverse_dir_transform(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                    METAL_ASQ_DEVICE const ShaderData *sd,
                                                    METAL_ASQ_THREAD float3 *D)
{
#ifdef __OBJECT_MOTION__
...
ccl_device_inline float3 object_location(METAL_ASQ_DEVICE const KernelGlobals *kg, METAL_ASQ_DEVICE const ShaderData *sd)
{
  if (sd->object == OBJECT_NONE)
...
ccl_device_inline float3 object_color(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
...
ccl_device_inline float object_pass_id(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0.0f;
...
ccl_device_inline float lamp_random_number(METAL_ASQ_DEVICE const KernelGlobals *kg, int lamp)
{
  if (lamp == LAMP_NONE)
    return 0.0f;
...
ccl_device_inline float object_random_number(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0.0f;
...
ccl_device_inline int object_particle_id(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0;
...
ccl_device_inline float3 object_dupli_generated(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return make_float3(0.0f, 0.0f, 0.0f);
...
ccl_device_inline float3 object_dupli_uv(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return make_float3(0.0f, 0.0f, 0.0f);
...
ccl_device_inline void object_motion_info(
                                          METAL_ASQ_DEVICE const KernelGlobals *kg, int object, METAL_ASQ_THREAD int *numsteps, METAL_ASQ_THREAD int *numverts, METAL_ASQ_THREAD int *numkeys)
{
  if (numkeys) {
...
ccl_device_inline uint object_patch_map_offset(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0;
...
ccl_device_inline float object_volume_density(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE) {
    return 1.0f;
  }
...
ccl_device_inline float object_volume_step_size(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE) {
...
ccl_device int shader_pass_id(METAL_ASQ_DEVICE const KernelGlobals *kg, METAL_ASQ_DEVICE const ShaderData *sd)
{
...
ccl_device_inline float object_cryptomatte_id(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0.0f;
...
ccl_device_inline float object_cryptomatte_asset_id(METAL_ASQ_DEVICE const KernelGlobals *kg, int object)
{
  if (object == OBJECT_NONE)
    return 0;
...
ccl_device_inline uint particle_index(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return kernel_tex_fetch(__particles, particle).index;
}

ccl_device float particle_age(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return kernel_tex_fetch(__particles, particle).age;
}

ccl_device float particle_lifetime(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return kernel_tex_fetch(__particles, particle).lifetime;
}

ccl_device float particle_size(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return kernel_tex_fetch(__particles, particle).size;
}

ccl_device float4 particle_rotation(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return kernel_tex_fetch(__particles, particle).rotation;
}

ccl_device float3 particle_location(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return float4_to_float3(kernel_tex_fetch(__particles, particle).location);
}

ccl_device float3 particle_velocity(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
  return float4_to_float3(kernel_tex_fetch(__particles, particle).velocity);
}

ccl_device float3 particle_angular_velocity(METAL_ASQ_DEVICE const KernelGlobals *kg, int particle)
{
...
ccl_device_inline float bvh_instance_push(
                                          METAL_ASQ_DEVICE const KernelGlobals *kg, int object, METAL_ASQ_THREAD const Ray *ray, METAL_ASQ_THREAD float3 *P, METAL_ASQ_THREAD float3 *dir, METAL_ASQ_THREAD float3 *idir)
{
...
ccl_device_inline float bvh_instance_pop(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                         int object,
                                         METAL_ASQ_THREAD const Ray *ray,
                                         METAL_ASQ_THREAD float3 *P,
                                         METAL_ASQ_THREAD float3 *dir,
                                         METAL_ASQ_THREAD float3 *idir,
                                         float t)
{
  if (t != FLT_MAX) {
    Transform tfm = object_fetch_transform(kg, object, OBJECT_INVERSE_TRANSFORM);
...
ccl_device_inline void bvh_instance_pop_factor(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                               int object,
                                               METAL_ASQ_THREAD const Ray *ray,
                                               METAL_ASQ_THREAD float3 *P,
                                               METAL_ASQ_THREAD float3 *dir,
                                               METAL_ASQ_THREAD float3 *idir,
                                               METAL_ASQ_THREAD float *t_fac)
{
  Transform tfm = object_fetch_transform(kg, object, OBJECT_INVERSE_TRANSFORM);
...
ccl_device_inline float bvh_instance_motion_push(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                 int object,
                                                 METAL_ASQ_THREAD const Ray *ray,
                                                 METAL_ASQ_THREAD float3 *P,
                                                 METAL_ASQ_THREAD float3 *dir,
...
ccl_device_inline float bvh_instance_motion_pop(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                int object,
                                                METAL_ASQ_THREAD const Ray *ray,
                                                METAL_ASQ_THREAD float3 *P,
                                                METAL_ASQ_THREAD float3 *dir,
...
ccl_device_inline void bvh_instance_motion_pop_factor(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                      int object,
                                                      METAL_ASQ_THREAD const Ray *ray,
                                                      METAL_ASQ_THREAD float3 *P,
                                                      METAL_ASQ_THREAD float3 *dir,
...
ccl_device_inline void object_position_transform_addrspace(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                           METAL_ASQ_DEVICE const ShaderData *sd,
                                                           METAL_ASQ_THREAD ccl_addr_space float3 *P)
{
  float3 private_P = *P;
...
ccl_device_inline void object_dir_transform_addrspace(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                      METAL_ASQ_DEVICE const ShaderData *sd,
                                                      METAL_ASQ_THREAD ccl_addr_space float3 *D)
{
  float3 private_D = *D;
...
ccl_device_inline void object_normal_transform_addrspace(METAL_ASQ_DEVICE const KernelGlobals *kg,
                                                         METAL_ASQ_DEVICE const ShaderData *sd,
                                                         METAL_ASQ_THREAD ccl_addr_space float3 *N)
{
  float3 private_N = *N;
